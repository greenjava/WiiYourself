...
{
public:
    WIIYOURSELF_EXPORT wiimote();
    WIIYOURSELF_EXPORT virtual ~wiimote();
...
public:
    ...
    // string versions
    WIIYOURSELF_EXPORT static const TCHAR* ReportTypeName[];
...
public:
    ...
    WIIYOURSELF_EXPORT virtual void ChangedNotifier(state_change_flags changed, const wiimote_state &new_state)
    {}

    // get the button name from its bit index (some bits are unused)
    WIIYOURSELF_EXPORT static const TCHAR* ButtonNameFromBit[TOTAL_BUTTON_BITS];
    ...
    WIIYOURSELF_EXPORT static const TCHAR* ClassicButtonNameFromBit[TOTAL_BUTTON_BITS];
    ...
    WIIYOURSELF_EXPORT static const unsigned FreqLookup[TOTAL_FREQUENCIES];
...
public:
    ...
    WIIYOURSELF_EXPORT bool Connect(unsigned wiimote_index = FIRST_AVAILABLE, bool force_hidwrites = false);

    // disconnect from the controller and stop reading data from it
    WIIYOURSELF_EXPORT void Disconnect();
    ...
    WIIYOURSELF_EXPORT void SetReportType(input_report type, bool continuous = false);
    ...
    WIIYOURSELF_EXPORT bool EnableMotionPlus();
    WIIYOURSELF_EXPORT bool DisableMotionPlus();
    ...
    //  are never modified).
    WIIYOURSELF_EXPORT void CalibrateAtRest();
    ...
    WIIYOURSELF_EXPORT state_change_flags RefreshState();

    // reset the wiimote (changes report type to non-continuous buttons-only,
    //                      clears LEDs & rumble, mutes & disables speaker)
    WIIYOURSELF_EXPORT void Reset();
    // set/clear the wiimote LEDs
    WIIYOURSELF_EXPORT void SetLEDs(BYTE led_bits); // bits 0-3 are valid
    // set/clear rumble
    WIIYOURSELF_EXPORT void SetRumble(bool on);
    // alternative - rumble for a fixed amount of time (asynchronous)
    WIIYOURSELF_EXPORT void RumbleForAsync(unsigned milliseconds);

    // *experimental* speaker support:
    WIIYOURSELF_EXPORT bool MuteSpeaker(bool on);
    WIIYOURSELF_EXPORT bool EnableSpeaker(bool on);
    WIIYOURSELF_EXPORT bool PlaySquareWave(speaker_freq freq, BYTE volume = 0x40);

    //  note: PlaySample currently streams from the passed-in wiimote_sample -
    //           don't delete it until playback has stopped.
    WIIYOURSELF_EXPORT bool PlaySample(const wiimote_sample &sample, BYTE volume = 0x40,
            speaker_freq freq_override = FREQ_NONE);

    //  16bit mono sample loading/conversion to native format:
    //   .wav sample
    WIIYOURSELF_EXPORT static bool Load16bitMonoSampleWAV(const TCHAR* filepath, wiimote_sample &out);
    //   raw 16bit mono audio data (can be signed or unsigned)
    WIIYOURSELF_EXPORT static bool Load16BitMonoSampleRAW(const TCHAR* filepath, bool _signed,
            speaker_freq freq, wiimote_sample &out);
    //   converts a 16bit mono sample array to a wiimote_sample
    WIIYOURSELF_EXPORT static bool Convert16bitMonoSamples(const short* samples, bool _signed,
    ...
    WIIYOURSELF_EXPORT void RecordState(state_history &events_out, unsigned max_time_ms =
            UNTIL_STOP, state_change_flags change_trigger = CHANGED_ALL);
    WIIYOURSELF_EXPORT void StopRecording();
